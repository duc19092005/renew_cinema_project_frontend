        ...
        <div className="p-6 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex justify-center">
                <div className={`p-1 rounded-2xl flex gap-1 ${
                    (theme === 'modern' || theme === 'dark') ? 'bg-[#0E0A20]/60 border border-white/5 shadow-2xl' : 'bg-gray-100 border border-gray-200'
                }`}>
                    <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                {/* 1. Source Selection */}
                <div className={`p-6 rounded-3xl border transition-all h-full ${
                    (theme === 'modern' || theme === 'dark') ? 'bg-[#0E0A20]/40 border-white/5 shadow-2xl' : 'bg-white border-gray-100 shadow-sm'
                }`}>
                    ...
                    <div className="relative">
                        <select
                            ...
                            className={`w-full h-12 pl-3 pr-10 rounded-xl appearance-none outline-none text-xs font-bold transition-all ${
                                (theme === 'modern' || theme === 'dark') ? 'bg-[#151225] border-white/5 text-white focus:border-cyan-500/50' : 'bg-gray-50 border-gray-200 focus:border-red-600/50'
                            } border`}
                        >
                ...
                {/* 2. Target Selection */}
                <div className={`p-6 rounded-3xl border transition-all h-full ${
                    (theme === 'modern' || theme === 'dark') ? 'bg-[#0E0A20]/40 border-white/5 shadow-2xl relative' : 'bg-white border-gray-100 shadow-sm relative'
                }`}>
                    <div className="flex items-center gap-3 mb-6">
                ...
                <div className={`p-6 rounded-3xl border transition-all lg:col-span-1 h-full ${
                    (theme === 'modern' || theme === 'dark') ? 'bg-[#0E0A20]/40 border-white/5 shadow-2xl' : 'bg-white border-gray-100 shadow-sm'
                }`}>
                    ...
                    <div className={`rounded-2xl overflow-hidden border ${
                        (theme === 'modern' || theme === 'dark') ? 'bg-[#151225]/40 border-white/5' : 'bg-gray-50/50 border-gray-100'
                    }`}>
                        <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
                            {loadingItems ? (
                                <div className="py-20 flex flex-col items-center justify-center opacity-40">
                                    <Loader2 className="w-8 h-8 animate-spin mb-2" />
                                    <p className="text-[10px] uppercase font-black tracking-widest">{t('Loading...')}</p>
                                </div>
                            ) : managedItems.length > 0 ? (
                                managedItems.map(item => (
                                    <div key={item.itemId} className={`group p-4 flex items-center gap-4 border-b last:border-0 transition-all ${
                                        (theme === 'modern' || theme === 'dark') 
                                            ? 'border-white/5 hover:bg-white/5' 
                                            : 'border-gray-100 hover:bg-gray-50'
                                    }`}>
                                        <div className={`p-2 rounded-lg ${(theme === 'modern' || theme === 'dark') ? 'bg-cyan-500/10 text-cyan-400' : 'bg-red-50 text-red-600'}`}>
                                            {transferType === 1 && <Building2 className="w-4 h-4" />}
                                            {transferType === 2 && <ShieldAlert className="w-4 h-4" />}
                                            {transferType === 3 && <Film className="w-4 h-4" />}
                                        </div>
                                        ...
                                        <button
                                            ...
                                            className={`flex items-center justify-center p-2 rounded-lg transition-all ${
                                                !targetUserId || executing
                                                    ? 'opacity-20 cursor-not-allowed'
                                                    : (theme === 'modern' || theme === 'dark')
                                                        ? 'bg-cyan-500 text-white hover:shadow-[0_0_15px_rgba(6,182,212,0.5)] active:scale-95'
                                                        : 'bg-red-600 text-white hover:bg-red-700'
                                            }`}
                                            title={!targetUserId ? t('Select a target manager first') : t('Transfer this item')}
                                        >
                                ...
                                <div className="py-20 flex flex-col items-center justify-center opacity-20">
                                    ...
                                    <p className="text-xs italic">{sourceUserId ? t('No items found') : t('Select source manager')}</p>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {/* Warning Section */}
            <div className={`mt-4 p-4 rounded-2xl flex items-start gap-3 border ${
                (theme === 'modern' || theme === 'dark') ? 'bg-amber-500/5 border-amber-500/20 text-amber-200' : 'bg-amber-50 border-amber-100 text-amber-800'
            }`}>
