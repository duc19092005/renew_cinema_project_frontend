...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange, isOpen, onToggle }) => {
  ...
  return (
    <>
      {/* Overlay */}
      <div
        className={`fixed inset-0 z-[100] lg:hidden transition-all duration-500 ease-in-out ${isOpen ? 'visible opacity-100 pointer-events-auto' : 'invisible opacity-0 pointer-events-none'
          }`}
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
          ...
          {user && (
            <div className="lg:hidden space-y-4 pb-4 border-b border-gray-500/10">
              ...
              {/* Preferences */}
              <div className={`pt-3 mt-1 border-t space-y-3 ${theme === 'dark' ? 'border-gray-800' : theme === 'modern' ? 'border-indigo-500/20' : 'border-gray-100'}`}>
                <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${theme === 'dark' ? 'text-gray-500' : theme === 'modern' ? 'text-indigo-400' : 'text-gray-400'}`}>{t('Preferences')}</p>
                <div className="flex items-center justify-between px-1">
                  <span className={`text-sm font-bold ${theme === 'dark' || theme === 'modern' ? 'text-white' : 'text-gray-900'}`}>{t('Language')}</span>
                  <LanguageSwitcher />
                </div>
                <div className="flex items-center justify-between px-1">
                  <span className={`text-sm font-bold ${theme === 'dark' || theme === 'modern' ? 'text-white' : 'text-gray-900'}`}>{t('Theme')}</span>
                  <div className="flex gap-2">
                    {(['light', 'dark', 'modern'] as const).map((tMode) => (
                      <button
                        key={tMode}
                        onClick={() => setTheme(tMode)}
                        className={`p-2 rounded-xl transition-all active:scale-90 ${theme === tMode ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/40' : 'bg-gray-500/10 text-gray-500 hover:bg-gray-500/20'}`}
                        title={tMode}
                      >
            ...
            <div className="space-y-2">
              ...
              {menuItems.map((item) => (
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all active:scale-95 ${activeTab === item.id ? theme === 'modern' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'bg-red-600 text-white shadow-lg shadow-red-600/20' : theme === 'dark' ? 'text-gray-400 hover:bg-gray-800 hover:text-white' : theme === 'modern' ? 'text-white/70 hover:bg-white/5 hover:text-white' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}
                >
