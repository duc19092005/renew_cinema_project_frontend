...
const HomePage: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-300 ${theme === 'dark' ? 'bg-black text-white' : theme === 'modern' ? 'bg-gradient-to-br from-[#0D081D] via-[#050A14] to-[#12081C] text-white' : 'bg-gray-50 text-gray-900'
      ...
      {/* --- HEADER --- */}
      <header className={`p-4 fixed top-0 left-0 right-0 z-[60] backdrop-blur-md border-b flex items-center justify-between h-16 px-4 sm:px-8 transition-all duration-500 ${isScrolled
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className={`lg:hidden p-2 rounded-lg transition-all active:scale-95 relative z-[70] ${theme === 'dark' ? 'hover:bg-gray-800 text-white' :
          ...
          <div
            className={`flex-shrink-0 ml-6 text-2xl sm:text-3xl font-black tracking-widest cursor-pointer transition-all hover:scale-105 active:scale-95 ${theme === 'modern'
          ...
          <div className="flex items-center gap-2">
            ...
            {!user ? (
              <button
                ...
                className={`lg:hidden px-3 py-1.5 rounded-lg font-bold text-xs transition-all active:scale-95 ${theme === 'modern' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'bg-red-600 text-white'
                  }`}
              >
                {t('header.login')}
              </button>
            ...
            <div className="hidden lg:flex items-center">
              {!user ? (
                <div className="flex items-center gap-2 sm:gap-3">
                  <button
                    onClick={() => navigate('/login')}
                    className={`px-4 py-2 rounded-lg font-bold text-sm transition-all hover:scale-105 active:scale-95 ${theme === 'modern' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'bg-red-600 text-white'
                      }`}
                  >
                    {t('header.login')}
                  </button>
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg font-bold text-sm border transition-all hover:scale-105 active:scale-95 ${theme === 'dark' || theme === 'modern' ? 'border-gray-700 text-white hover:bg-white/5' : 'border-gray-200 text-gray-700 hover:bg-gray-50'
                      }`}
                  >
                    {t('header.register')}
                  </button>
      ...
      {/* MOBILE SIDEBAR */}
      <div
        className={`fixed inset-0 z-[100] lg:hidden transition-all duration-500 ease-in-out ${isMobileMenuOpen ? 'visible opacity-100 pointer-events-auto' : 'invisible opacity-0 pointer-events-none'
          }`}
      >
        <div
        ...
        />
        <div className={`absolute inset-y-0 left-0 z-[110] w-[280px] transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) transform ${isMobileMenuOpen ? 'translate-x-0 scale-100' : '-translate-x-full scale-95'
            ...
            {!user ? (
              <div className="space-y-4 pb-4 border-b border-gray-500/10">
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-xl font-bold border transition-all active:scale-95 text-sm uppercase tracking-[0.1em] ${theme === 'dark' || theme === 'modern' ? 'border-gray-700 text-white hover:bg-white/5' : 'border-gray-200 text-gray-700 hover:bg-gray-50'
                    }`}
                >
                  {t('header.register')}
                </button>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 mb-12">
            {nowShowing.map((movie, index) => (
              <div
                key={movie.movieId}
                className={`group rounded-xl overflow-hidden shadow-lg border transition-all hover:-translate-y-1 cursor-pointer opacity-0 animate-reveal ${`stagger-${(index % 5) + 1}`
                  } ${theme === 'dark'
